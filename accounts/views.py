...
def login_modal(request):
    ...
    if user:
        ...
        messages.success(request, (
            f"Welcome {user.first_name}, you are logged in!"))
        return redirect(reverse('index'))
    else:
...
def user_profile(request):
    ...
    user_liked_events = (Event.objects.filter(id__in=user_liked)
                         .order_by('event_date_begins'))
    user_participant = Participant.objects.filter(user=user).values('event')
    user_participant_events = (Event.objects.filter(id__in=user_participant)
                               .exclude(event_date_ends__lt=datetime.date
                               .today()).order_by('event_date_begins'))
    user_participated_events = (Event.objects.filter(id__in=user_participant)
                                .exclude(event_date_ends__gt=datetime.date
                                .today()).order_by('-event_date_begins'))
    user_hosted_events = (Event.objects.filter(event_host=user)
                          .exclude(event_date_ends__lt=datetime.date.today())
                          .order_by('event_date_begins'))
    user_past_hosted_events = (Event.objects.filter(event_host=user)
                               .exclude(event_date_ends__gt=datetime.date
                               .today()).order_by('-event_date_begins'))
